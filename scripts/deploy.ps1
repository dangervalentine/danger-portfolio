...
# optimisation, not an artifact -- except .next/cache/images: the image
# optimizer keys that by src URL with no invalidation, so replacing a file
# under /public at the same path would keep serving the old optimized copy.
...
if (Test-Path $nextDir) {
    ...
    $imageCache = Join-Path $nextDir "cache\images"
    if (Test-Path $imageCache) {
        Remove-Item $imageCache -Recurse -Force -ErrorAction SilentlyContinue
    }
    Write-Host "  - cleared stale .next output and image cache (kept the rest of .next/cache)"
}
